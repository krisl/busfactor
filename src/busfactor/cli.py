...
def build_rules_engine(rules_cfg: dict[str, dict[str, Any]], verbose: bool = False) -> RulesEngine | None:
    ...
    for target, rule_def in rules_cfg.items():
        if "follow" in rule_def:
            ...
            delay_ms = int(rule_def.get("follow_delay_ms", 0))
            rules.append(FollowRule(target=str(target), source=str(rule_def["follow"]), inverted=inverted, delay_ms=delay_ms))
...
def resolve_runtime(cfg: S7MonitorConfig) -> ResolvedRuntime:
    ...
    if cfg.variables or cfg.field_vars:
        ...
        if protocol == "eip":
            read_groups = build_eip_read_groups(
                input_assembly=cfg.input_assembly or 101,
                input_size=cfg.input_size or 32,
                output_assembly=cfg.output_assembly or 100,
                output_size=cfg.output_size or 32,
            )
        else:
